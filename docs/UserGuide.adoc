...
More importantly, IB is *optimized for those who prefer to work with a Command Line Interface* (CLI) while still having the benefits of a Graphical User Interface (GUI). If you can type fast, IB can get your candidates management tasks done faster than traditional GUI apps. +
...
.  Set up is complete. Try some example commands shown here:
...
Views the person whose email is the same as given email. +
Format: `view email`
****
...
Return the person whose email is `john@gmail.com` +
...
=== Job Posting
Add job requirements to find out qualified candidates.

Format: `addjob [SKILLS]`

=== Resume Auto Scanning
Easily filter candidates whose resume having desirable keywords. +
...
View LinkedIn profile of the person identified by the index number used in the last person listing. +
...
*Q*: How do I check if Infinty Book is compatible with my computer? +
